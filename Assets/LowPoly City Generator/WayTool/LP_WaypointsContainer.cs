...
namespace LPCG
{
    public class LP_WaypointsContainer : MonoBehaviour
    {
        ...
        public LP_WaypointsContainer[] doNotConnectTo;

        [Range(10, 40)]
        public float limitNodeDistance = 25;  //Maximum distance between the end of one lane to the bridge of the other for automatic linking
        ...
        [HideInInspector]
        public LP_WaypointsContainer[] nextWay0;
        [HideInInspector]
        public LP_WaypointsContainer[] nextWay1;
        [HideInInspector]
        ...
        [HideInInspector]
        public LP_WaypointsContainer[] directConnectSide = new LP_WaypointsContainer[2];

        Vector3 nodeBegin;
        Vector3 nodeEnd;

        [HideInInspector]
        public LP_TrafficSystem.WpData wpData;
            ...
            for (int idx = 1; idx >= 0; idx--)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    if (pathDistance < 3)
                    {
                        ...
                        if (idx == 0)
                        {
                            nextWay0 = new LP_WaypointsContainer[1];
                        ...
                        {
                            nextWay1 = new LP_WaypointsContainer[1];
        ...
        public void ResetWay()
        {
            ...
            nextWay0 = new LP_WaypointsContainer[0];
            nextWay1 = new LP_WaypointsContainer[0];
            ...
            for (int idx = 1; idx >= 0; idx--)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    if (pathDistance < limitNodeDistance && pathDistance >= 3)// && (tf01Script.tsGroup[i] == "" || tf01Script.tsGroup[i] == group || group == ""))
                    {
                        ...
                        LP_WaypointsContainer wpc = wpData.tsParent[i];
                ...
                LP_WaypointsContainer[] _NextWays = new LP_WaypointsContainer[qt];
                int[] _NextWaysSide = new int[qt];

                for (int i = 0; i < qt; i++)
                {
                    _NextWays[i] = (LP_WaypointsContainer)arrParent[i];
        ...
        public bool TestDoNotConnectTo(LP_WaypointsContainer t)
        {
...
#if UNITY_EDITOR
                if (!trafficSystem)
                    trafficSystem = (LP_TrafficSystem)AssetDatabase.LoadAssetAtPath("Assets/LowPolyCity/Traffic System/Traffic System.prefab", (typeof(LP_TrafficSystem)));
#endif

                if (!trafficSystem)
                    Debug.LogError("Traffic System.prefab was not found in 'Assets/LowPolyCity/Traffic System'");
            }
        ...
        public Transform GetNodeZeroOldWay(int side)
        {
            ...
            return (side == 0) ? nodeZeroCar0.GetComponent<LP_TrafficCar>().myOldWay : nodeZeroCar1.GetComponent<LP_TrafficCar>().myOldWay;
        ...
        public bool BookNodeZero(LP_TrafficCar trafficCar)
        {
            ...
            if (trafficCar.sideAtual == 0)
            {
                ...
                {
                    if (trafficCar.nodeSteerCarefully2 == false && trafficCar.nodeSteerCarefully == false && nodeZeroCar0.GetComponent<LP_TrafficCar>().Get_avanceNode() && trafficCar.GetBehind() != nodeZeroCar0)
                    {
                    ...
                    return ((nodeZeroCar0 == trafficCar.transform) || (nodeZeroWay0 == trafficCar.myOldWay && trafficCar.myOldSideAtual == nodeZeroCar0.GetComponent<LP_TrafficCar>().myOldSideAtual));
                }
            ...
            else
            {
                ...
                {
                    if (trafficCar.nodeSteerCarefully == false && nodeZeroCar1.GetComponent<LP_TrafficCar>().Get_avanceNode() && trafficCar.GetBehind() != nodeZeroCar1)
                    {
                        SetNodeZero(1, trafficCar.myOldWay, trafficCar.transform, true);
                    }

                    //The starting node of the path is already reserved for another car. So the car that called this procedure must wait.                
                    return ((nodeZeroCar1 == trafficCar.transform) || (nodeZeroWay1 == trafficCar.myOldWay && trafficCar.myOldSideAtual == nodeZeroCar1.GetComponent<LP_TrafficCar>().myOldSideAtual));
                }
