...
namespace LPCG
{
    public class LP_TrafficCar : MonoBehaviour
    {
        ...
        public GameObject BreakLight = null;
        ...
        [HideInInspector]
        public LP_WaypointsContainer atualWayScript;
        ...
        [HideInInspector]
        public LP_WaypointsContainer myOldWayScript = null;
        ...
        //[HideInInspector]
        public LP_TrafficSystem tSystem;
        ...
        public void Configure()
        {
            ...
            if (!BreakLight)
                if(transform.Find("Break-Light"))
                    BreakLight = transform.Find("Break-Light").gameObject;

            if (!LightLeft)
                if (transform.Find("Light-Left"))
                    LightLeft = transform.Find("Light-Left").gameObject;

            if (!LightRight)
                if (transform.Find("Light-Right"))
            ...
            {
                Debug.LogError("Add a collider to the vehicle body and then click 'Generate WheelColliders' again.");
            }
        ...
        void Start()
        {
            ...
            if(player && !FindObjectOfType<LP_TrafficSystem>())
        ...
        public void Init()
        {
            ...
            atualWayScript = atualWay.GetComponent<LP_WaypointsContainer>();
        ...
        bool CheckBookAllPathOptions(LP_WaypointsContainer wayScript, int side)
        {

            int total;
            int wSide;
            LP_WaypointsContainer wScript;
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                if (wScript)
                {
                    if ((wScript.GetNodeZeroCar(wSide) != null && wScript.GetNodeZeroCar(wSide) != transform) && wScript.GetNodeZeroOldWay(wSide) != myOldWay && (!Get_avanceNode() || !wScript.GetNodeZeroCar(wSide).GetComponent<LP_TrafficCar>().Get_avanceNode()))
                        return false;
                }
                else
                {
        ...
        bool BookAllPathOptions(LP_WaypointsContainer wayScript, int side, bool book = true)
        {
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                if (book)
                {
                    bool force = wScript.GetNodeZeroCar(wSide) && wScript.GetNodeZeroCar(wSide).GetComponent<LP_TrafficCar>().Get_avanceNode();
...
private void VerificaPoints()
            ...
            if (distanceToNode < 5)
            {
                ...
                else
                {
                    ...
                    if (myOldWayScript.oneway && !myOldWayScript.doubleLine)
                    {
                        ...
                        nodeSteerCarefully = (atualWayScript.rightHand != 1 && a < 340 && a > 270) || (atualWayScript.rightHand == 1 && a > 20 && a < 90);
        ...
        float FixedRaycasts()
        {
            ...
            if (rStop > 0 && speed < 2)
            {
                ...
                else if (hit.transform.GetComponent<LP_TrafficCar>())
                {
                    StatusCar st = hit.transform.GetComponent<LP_TrafficCar>().status;
        ...
        */
        ...
        float VerifyTraffic(int t, float mts = 12)
        {
            ...
            if (Physics.Raycast(node0, node1 - node0, out hit2, mts))
                if (hit2.transform.GetComponent<LP_TrafficCar>())
                {
                    if (hit2.transform.GetComponent<LP_TrafficCar>().GetSpeed() < 8)
        ...
        bool SelfDestructWhenAwayFromThePlayer()
        {
